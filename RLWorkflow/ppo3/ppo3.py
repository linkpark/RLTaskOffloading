...
from RLWorkflow.common.tf_util import get_session
from RLWorkflow.common.runners import AbstractEnvRunner
...
from RLWorkflow.ppo3.policies import build_policy
...
# S2S model, trajectories learning
class S2SModel(object):
    ...
    train():
    - Make the training part

    save/load():
    - Save load the model
    """
    def __init__(self, *, policy, ent_coef, vf_coef, max_grad_norm):
        sess = get_session()
        # sequential action
        ...
        with tf.variable_scope('ppo3_model', reuse=tf.AUTO_REUSE):
            # create two models( policies)
            # act_model that is used for sampling

            # train model for training
            train_model = policy(decoder_inputs=decoder_input,
                                 decoder_full_length=action_length,
                                 decoder_targets=action)
            act_model = policy(decoder_inputs=decoder_input,
                               decoder_full_length=action_length,
                               decoder_targets=action)
...
class S2SModel_Back(object):
    def __init__(self, *, ob, ob_length, ent_coef, vf_coef, max_grad_norm):
        ...
        train_model = SelfAttentionSequencePolicy("pi", hparams, reuse=True, encoder_inputs=ob,
                 encoder_lengths=ob_length,
                 decoder_inputs=decoder_input,
                 decoder_full_length=action_length,
                 decoder_targets=action)
        act_model = SelfAttentionSequencePolicy("oldpi", hparams, reuse=False, encoder_inputs=ob,
                                     encoder_lengths=ob_length,
                                     decoder_inputs=decoder_input,
                                     decoder_full_length=action_length,
                                     decoder_targets=action)
...
class Runner():
    ...
    def sample_eval(self):
        ...
        for encoder_batch, encoder_length, decoder_lengths, task_graph_batch \
            ...
            runnint_cost.append(self.env.get_running_cost(action_sequence_batch=actions,
    ...
    def greedy_eval(self):
        ...
        for encoder_batch, encoder_length, decoder_lengths, task_graph_batch \
            ...
            runnint_cost.append(
                self.env.get_running_cost(action_sequence_batch=actions, task_graph_batch=task_graph_batch))
...
def learn(network, env, total_timesteps, eval_env = None, seed=None, nsample_episode=30, nsteps=2048, ent_coef=0.01, lr=1e-4,
    ...
    if eval_env is not None:
        eval_runner = Runner(env=eval_env, model=model, nepisode=1, gamma=gamma, lam=lam)
    ...
    for update in range(1, nupdates+1):
        ...
        if eval_env is not None:
            running_cost = eval_runner.sample_eval()
            greedy_running_cost = eval_runner.greedy_eval()

        run_time = np.mean(running_cost)
        greedy_run_time = np.mean(greedy_running_cost)
        ...
        if update % log_interval == 0 or update == 1:
            ...
            logger.logkv('run time cost', run_time)
            logger.logkv('greedy run time cost', greedy_run_time)
            logger.logkv('heft time cost', eval_env.heft_cost)
...
if __name__ == "__main__":
    env = WorkflowEnvironment(batch_size=100, graph_number=100, graph_file_path="../data/random20/random.20.",
                              is_xml=False, time_major=False)

    eval_env = env

    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        mean_reward_track = learn(network="default", env=env, eval_env=eval_env, nsample_episode=30,
                                  max_grad_norm=1.0, noptepochs=4,
                                  total_timesteps=80000, lr=5e-4)

    x = np.arange(0, len(mean_reward_track), 1)
    np.save("mean_reward_4optepch.txt", mean_reward_track)
