...
from RLWorkflow.offloading_ddqn.seq2seq_qnet import Seq2seqQNet
...
class Seq2SeqDDQN(object):
    def __init__(self, hparams, ob_dim, gamma, max_grad_norm):
        ...
        self.encoder_length =tf.placeholder(dtype=tf.int32, shape=[None])
        self.decoder_input = tf.placeholder(tf.int32, [None, None])
        self.action = tf.placeholder(tf.int32, [None, None])
        self.action_length = tf.placeholder(tf.int32, [None])

        # placeholder for target q net
        self.target_ob_input = tf.placeholder(dtype=tf.float32, shape=[None, None, ob_dim])
        self.target_encoder_length = tf.placeholder(dtype=tf.int32, shape=[None])
        self.target_decoder_input = tf.placeholder(tf.int32, [None, None])
        self.target_action = tf.placeholder(tf.int32, [None, None])
        self.target_action_length = tf.placeholder(tf.int32, [None])
        ...
        self.q_net = Seq2seqQNet("qnet", hparams, reuse=True,
                            encoder_inputs=self.ob_input,
                            encoder_lengths=self.encoder_length,
                            decoder_inputs=self.decoder_input,
                            decoder_full_length=self.action_length,
                            decoder_targets=self.action)

        self.target_q_net = Seq2seqQNet("target_qnet", hparams, reuse=False,
                            encoder_inputs=self.target_ob_input,
                            encoder_lengths=self.target_encoder_length,
                            decoder_inputs=self.target_decoder_input,
                            decoder_full_length=self.target_action_length,
                            decoder_targets=self.target_action)
    ...
    def train(self, sess, learning_rate, obs, obs_length,
              rewards, decoder_inputs, actions, decoder_full_length,
              target_next_q_values):

        td_map = {self.lr: learning_rate,
                  self.ob_input: obs, self.encoder_length: obs_length,
                  self.r: rewards, self.decoder_input: decoder_inputs,
                  self.action_length: decoder_full_length,
                  self.action: actions,
...
class Runner(object):
    ...
    def run(self, epsilon_threshold=0.1):
        ...
        for task_graph_batch, encoder_batch, encoder_length, \
            ...
            for _ in range(self.nepisode):
                sample_actions, greedy_actions = self.model.step(encoder_input_batch=encoder_batch,
    ...
    def evaluate(self):
        ...
        for encoder_batch, encoder_length, decoder_lengths, task_graph_batch \
            ...
            _, actions = self.model.step(encoder_input_batch=encoder_batch,
                                         decoder_full_length=decoder_lengths)
        ...
        if update % train_freq == 0:
            ...
            for _ in range(update_numbers):
                ...
                for replay_buffer in replay_buffers:
                    ...
                    q_loss, _ = ddqn_model.train(sess, learning_rate=lr, obs=ob_seq, obs_length=decoder_full_length,
                                  rewards=rew_seq, decoder_inputs=dec_input_seq, actions=ac_seq,
...
if __name__ == "__main__":
    ...
    env = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=500, graph_number=500,
                                graph_file_paths=["../offloading_data/offload_random10/random.10.",
                                                  "../offloading_data/offload_random15/random.15.",
                                                  "../offloading_data/offload_random20/random.20.",
                                                  "../offloading_data/offload_random25/random.25.",
                                                  "../offloading_data/offload_random30/random.30.",
                                                  "../offloading_data/offload_random35/random.35.",
                                                  "../offloading_data/offload_random40/random.40.",
                                                  "../offloading_data/offload_random45/random.45.",
                                                  "../offloading_data/offload_random50/random.50.",
                                                  ],
                                time_major=False,
    ...
    eval_env_1 = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=100, graph_number=100,
                                       graph_file_paths=[
                                           "../offloading_data/offload_random10_test/random.10."],
                                       time_major=False,
                                       lambda_t=lambda_t, lambda_e=lambda_e)
    ...
    eval_env_2 = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=100, graph_number=100,
                                       graph_file_paths=[
                                           "../offloading_data/offload_random15_test/random.15."],
                                       time_major=False,
                                       lambda_t=lambda_t, lambda_e=lambda_e)
    ...
    eval_env_3 = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=100, graph_number=100,
                                       graph_file_paths=[
                                           "../offloading_data/offload_random20_test/random.20."],
                                       time_major=False,
                                       lambda_t=lambda_t, lambda_e=lambda_e)
    ...
    eval_env_4 = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=100, graph_number=100,
                                       graph_file_paths=[
                                           "../offloading_data/offload_random25_test/random.25."],
                                       time_major=False,
                                       lambda_t=lambda_t, lambda_e=lambda_e)
    ...
    eval_env_5 = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=100, graph_number=100,
                                       graph_file_paths=[
                                           "../offloading_data/offload_random30_test/random.30."],
                                       time_major=False,
                                       lambda_t=lambda_t, lambda_e=lambda_e)
    ...
    eval_env_6 = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=100, graph_number=100,
                                       graph_file_paths=[
                                           "../offloading_data/offload_random35_test/random.35."],
                                       time_major=False,
                                       lambda_t=lambda_t, lambda_e=lambda_e)
    ...
    eval_env_7 = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=100, graph_number=100,
                                       graph_file_paths=[
                                           "../offloading_data/offload_random40_test/random.40."],
                                       time_major=False,
                                       lambda_t=lambda_t, lambda_e=lambda_e)
    ...
    eval_env_8 = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=100, graph_number=100,
                                       graph_file_paths=[
                                           "../offloading_data/offload_random45_test/random.45."],
                                       time_major=False,
                                       lambda_t=lambda_t, lambda_e=lambda_e)
    ...
    eval_env_9 = OffloadingEnvironment(resource_cluster=resource_cluster, batch_size=100, graph_number=100,
                                       graph_file_paths=[
                                           "../offloading_data/offload_random50_test/random.50."],
                                       time_major=False,
                                       lambda_t=lambda_t, lambda_e=lambda_e)
