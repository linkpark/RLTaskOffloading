...
class Seq2seqPolicy(object):
    def __init__(self, name, hparams, reuse, encoder_inputs, decoder_inputs, decoder_full_length, decoder_targets):
        ...
        with tf.variable_scope(name, reuse=self.reuse):
            ...
            self.q = tf.layers.dense(self.decoder_logits, self.n_features, activation=None, reuse=tf.AUTO_REUSE, name="qvalue_layer")
            self.v = tf.reduce_sum(self.pi * self.q, axis=-1)
    ...
    def sample_from_current_policy(self, sess, encoder_input_batch, decoder_full_length):
        return sess.run([self.sample_decoder_prediction, self.sample_v],
                        feed_dict={
                            self.encoder_inputs: encoder_input_batch,
                            self.decoder_full_length: decoder_full_length
                        })

    def greedy_predict(self, sess, encoder_input_batch, decoder_full_length):
