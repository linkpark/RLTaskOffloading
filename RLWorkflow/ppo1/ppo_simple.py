...
from RLWorkflow.ppo1.seq2seq_policy import Seq2seqPolicy
...
def ppo_learning(clip_param, entcoeff, gamma, optim_stepsize, lam,
                 adam_epsilon=1e-5, max_episodes=0, max_timesteps=0,
    ...
    vf_loss = tf.reduce_mean(tf.square(pi.v - ret))
    total_loss = pol_surr + pol_entpen + vf_loss
    losses = [pol_surr, pol_entpen, vf_loss, meankl, meanent]
    loss_names = ["pol_surr", "pol_entpen", "vf_loss", "kl", "ent"]

    var_list = pi.get_trainable_variables()
    lossandgrad = U.function([ob, decoder_inputs, decoder_full_length, ac, atarg, ret, lrmult], losses + [U.flatgrad(total_loss, var_list)])
    adam = MpiAdam(var_list, epsilon=adam_epsilon)

    assign_old_eq_new = U.function([],[], updates=[tf.assign(oldv, newv)
        for (oldv, newv) in zipsame(oldpi.get_variables(), pi.get_variables())])

    compute_losses = U.function([ob, decoder_inputs, decoder_full_length, ac, atarg, ret, lrmult], losses)
    ...
    while True:
        ...
        for _ in range(optim_epochs):
            ...
            for batch in d.iterate_once(optim_batchsize):
                ...
                *newlosses, g = lossandgrad(encoder_input, decoder_input, decoder_full_length, decoder_target, atarg, tdlamret, cur_lrmult)
        ...
        for batch in d.iterate_once(optim_batchsize):
            ...
            if env.time_major:
                ...
                atarg = np.array(batch["atarg"]).swapaxes(0, 1)
                tdlamret = np.array(batch["vtarg"]).swapaxes(0, 1)
            newlosses = compute_losses(encoder_input, decoder_input, decoder_full_length, decoder_target, atarg, tdlamret, cur_lrmult)
            losses.append(newlosses)
        ...
        end_time = time.time()
        logger.info("time consume is {}".format(end_time - start_time))
...
if __name__ == "__main__":
    ...
    goal_track, reward_track = ppo_learning(clip_param=0.2, entcoeff=0.01, gamma=0.99, optim_stepsize=5e-4, lam=0.95,
                                           max_episodes=2000, max_timesteps=100, optim_epochs=4, optim_batchsize=500,
