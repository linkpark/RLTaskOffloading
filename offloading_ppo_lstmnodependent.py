...
class LSTMPPOModel(object):
    def __init__(self, obs_dim, hparams, ent_coef, vf_coef, max_grad_norm):
        sess = get_session()
        # sequential state
        ...
        train_model = LSTMPolicy(name='pi', hparams=hparams, reuse=True, encoder_inputs=obs)
        act_model = LSTMPolicy(name='oldpi', hparams=hparams, reuse=True, encoder_inputs=obs)
...
def learn(network, env, hparams, total_timesteps, eval_envs = None, seed=None, nupdates=1000, nsample_episode=30, nsteps=2048, ent_coef=0.01, lr=1e-4,
...
if __name__ == "__main__":
    ...
    logger.configure('./log/all-graph-lstm-no-dependency-latency-optimal', ['stdout', 'json', 'csv'])
