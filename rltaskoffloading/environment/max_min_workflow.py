...
def schedule(avaliable_task_list, task_graph, cluster, plan):
    while(len(avaliable_task_list) > 0 ):
        ...
        for task, rt in enumerate(RT):
            max_min_vector.append(ECT[task, rt])

        task_index = np.argmax(np.array(max_min_vector))
...
def max_min_policy(cluster, task_graph):
...
if __name__ == "__main__":
    ...
    task_graph = TaskGraph("../data/random30/random.30.0.gv", is_xml=False, is_matrix=False)

    plan = max_min_policy(resource_cluster, task_graph)
    ...
    print("max min cost is {}".format(cost))
